...
## `jetstream { limits { … } }` — the server-wide consumer and batch ceilings

`JSLimitOpts` (`opts.go:375–384` at v2.14.6): `max_request_batch` (a pull may not ask for more; also
the default `max_batch` of a new consumer), `max_ack_pending` (clamps every consumer's default and
ceiling), `max_ha_assets` (R>1 streams and consumers a server may hold), `max_duplicate_window`
(clamps the 2-minute default down), and the batch keys `max_batch_inflight_per_stream`,
`max_batch_inflight_total`, `max_batch_size`, `max_batch_timeout` (defaults 50 / 1000 / 1000 / 10 s
for atomic batches, `stream.go:446–455`). The account-level twin is `accounts.<name>.jetstream {
max_ack_pending }` beside the four storage keys above, and the stream-level one is
`consumer_limits { inactive_threshold, max_ack_pending }`; how the three layers combine into a
consumer's effective value is on [[stream-and-consumer-config]] (source:
