...
## What can change after creation, and what the server fills in

Every field with its default, its CLI flag and whether an update may change it is tabled on
[[stream-and-consumer-config]]. The short form, from `checkNewConsumerConfig` at v2.14.6 and three
passes of raw updates on the binary (source: [[s-nats-server-stream-consumer-config]],
[[s-nats-server-config-mutability-observed]]): **fixed** — `durable_name` / `name`,
`deliver_policy` and its `opt_start_seq` / `opt_start_time`, `ack_policy`, `replay_policy`,
`max_waiting`, `mem_storage`, `idle_heartbeat`, `flow_control`, and pull ↔ push (`can not update
pull consumer to push based`); **free** — `description`, `ack_wait`, `max_deliver`, `backoff`,
`filter_subject` / `filter_subjects`, `max_ack_pending`, `max_batch` / `max_expires` / `max_bytes`,
`inactive_threshold`, `num_replicas`, `metadata`, `pause_until`, `headers_only`, `sample_freq`,
`rate_limit_bps`. `nats consumer edit` offers flags for exactly the free set
([[s-nats-cli-help-0.4.0]]). Two defaults worth knowing beyond the table on this page: the API's
`ack_policy` default is **`none`** (the CLI's `--ack` default is `explicit`), and `ack_wait` and
`max_ack_pending` are filled in **only** for `explicit` and `all` — an ephemeral created with
`ack_policy: none` came back with neither, and with `inactive_threshold` 5 s and `max_waiting` 512.
...
the 2.10 release bodies patch consumers from v2.10.0 on and none records the arrival, which is
...
- **2.14.0** (source: [[s-relnotes-2.14]]): **`$JS.API.CONSUMER.RESET.<stream>.<consumer>`** (#7489);
  the **`AckFlowControl`** policy (JSON `flow_control`) for the durable sourcing consumers — the
  server at v2.14.6 requires it to be push-based with `flow_control` on, a heartbeat of exactly `1s`,
  a positive `max_ack_pending`, and no `ack_wait`, `backoff` or `max_deliver` (`consumer.go:758–780`);
  a `no_wait` pull with no expiry and nothing pending answers `404 No Messages` (#7466, also 2.11.11);
  **invalid or divergent consumer state is reset to the stream's on startup** "i.e. after unclean
  shutdowns" (#7692); consumer info and list requests are queued behind create/update/delete (#7898).
- **2.14.1**: the drifted-redelivered-state fixes (#8102, #8156, #8168, #8126, #8198 — above, and
  [[consumer-keeps-redelivering]]); pending calculated only on consumer leaders (#8172).
  **2.14.2**: a consumer lock released after a start-sequence error (#8230). **2.14.3**: ack
  subscriptions match names containing `%` (#8301); the inactive-delete grace period and pull
  `max_bytes` budgeting fixed (#8314); memory-store pending no longer overcounted for
  `last_per_subject` (#8313). **2.14.4**: changing the storage type returns an error (#8382); a
  consumer created right after its clustered stream no longer gets `stream not found` (#8410); reset
  responses no longer dropped through a service import (#8407).
- **2.14.6**: **replicated consumers no longer stuck after a leader change** — "a flow control
  problem" (#8488); **"various consumer create issues that could destroy the state of an existing
  consumer with the same name"** (#8491); delivery counts no longer underflow below zero (#8512);
  the direct or sourcing status cannot be changed by an update (#8478); consumer tiers distinguished
  when enforcing limits (#8484); `AckFlowControl` consumers on a WorkQueue upstream no longer ack
...
## What a client sees when the consumer leader moves

A consumer's leader can move without the application noticing much. Measured on 2.14.6 with a
durable pull consumer on an R3 stream, fetching one message at a time while the consumer leader's
node was stopped (source: [[s-nats-server-client-lifecycle-observed]], runs E8–E9):

- **one fetch of 120 failed**, with `nats: no responders available for request`, in 17 ms — the
  `$JS.API` request that landed in the leadership window. The next fetch succeeded. See
  [[nats-timeout]] for why that is *no responders* and not a timeout;
- messages already delivered and **un-acked** when the leader moved were redelivered on a later
  fetch with `tries: 2`, under new consumer sequences — see [[ack-and-redelivery]];
- `num_redelivered` still read **0** on the new leader 35 s after the move, before those
  redeliveries were fetched.

The connection's side of the same event — reconnect, drain, what the client prints — is on
...
## A filter is a seek, not a scan

The fact the whole "one big stream, many filtered consumers" design rests on, and it was asked in
exactly the form that worries an architect: a stream on `foo.bar.>` holding a million messages, a new
consumer filtering `foo.bar.baz.xyz` with **one** matching message somewhere in the middle — does it
walk the million? (source: [[s-gh-3405-consumer-filtering-performance]])
...
- **The cost of a filtered read tracks the span of the filter's subjects in the sequence space**, not
  the stream's size — the index gives first and last per subject, and the read walks the blocks
  between them ([[filestore-layout]]).
- **When that span is the whole stream and its interior is mostly deletes, the seek degenerates into a
  scan.** That is the KV hot-key shape, and it is measured on
  [[consumer-slow-on-a-sparse-stream]] rather than assumed.

Two caveats on the quote. It is dated **2022 and 2.9** — five minors ago — and the asker never ran the
benchmark he promised, so nothing was ever measured in public; and it says *efficient*, not *free*. The
...
it, the answer was that the premise is wrong: "It's not because a stream is larger that delivery of
messages to consumers takes longer" (source: [[s-gh-6571-source-mirror-or-one-stream]]). A fast
consumer and a slow consumer on one stream is a supported shape, not a compromise — each has its own
sequence, its own ack floor and its own `max_ack_pending`.

What a slow consumer *does* hold is the **data**, and only under `interest` or `workqueue` retention,
where the stream cannot delete a message until every interested consumer is finished with it
([[retention-policies]]). Under `limits` it holds nothing, and the only cost of the big stream is
per-subject index memory — a subject-design consequence ([[subjects-and-wildcards]]).
