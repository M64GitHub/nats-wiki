...
## What bites you — the CLI is silent about auth failures

Under an expired credential the CLI does not stop and does not say why. Measured on 0.4.0 against a
user JWT that lapsed 20 s into the run (source: [[s-nats-server-client-faults-observed]], B2): in
**45 seconds it printed two lines** —
...
— while the server rejected it **eleven times** (`-ERR User Authentication Expired` once, then
`-ERR Authorization Violation` ten times). It never named the reason and never gave up, because it
sets both `IgnoreAuthErrorAbort()` and unlimited reconnects
(source: [[s-docs-resilient-clients-tls-and-auth]], and the source read in
[[s-nats-cli-reconnect]]).

Two consequences for anyone using the CLI to diagnose:

- **"The CLI still connects" is not evidence a credential is valid**, and "the CLI is quiet" is not
  evidence nothing is wrong. Read the *server's* log, or `/connz?state=closed`, whose `reason` field
  separates `Authentication Expired` from `Authentication Failure` and `Revocation` — information
  the wire never carries (see [[monitoring-endpoints]]).
- **The CLI is not a witness for what an application would do.** A library client on the defaults
  closes within seconds of the same expiry — [[connection-closed-after-auth-error]] has the
  measured timings. [[client-defaults]] has the whole comparison.

The same asymmetry applies to a **client-side slow consumer**: the CLI registers its async error
handler twice and the `--trace`-gated registration wins, so `nats sub` prints nothing at all when it
drops messages (source: [[s-nats-cli-reconnect]]). Reproduce that one in a client library, not here —
[[slow-consumer-in-the-client]].


## `nats service` — the six subcommands, as run on 0.4.0

`micro` is an alias for `service`; ADR-32 asks tooling to use "service" (source:
[[s-adr-32-service-api]]). All six wrap requests on the `$SRV` tree of the
[[services-framework]]:

```
nats service list [<name>] [--json]         # every service and instance, gathered by deadline
nats service info <name> [<id>] [--json] [--endpoint=REGEX]
nats service stats <name> [<id>] [--json]
nats service ping [<name>]                  # one line per instance, with its rtt
nats service request <name> <endpoint> [payload] [-H K:V] [--raw] [--count=N]
nats service serve <name> [-H K:V]          # a demo echo service; not a way to host one
```

Three things bite (source: [[s-nats-server-services-observed]], runs A1, A3, A11):

- **`nats service info` shows one instance even when several are running.** It takes the first reply
  and stops — the "five running instances appear as one" mistake
  ([[s-docs-services-discovery-and-stats]]) living in the CLI. `list` and `ping` gather by deadline
  and are the honest ones; `info <name> <id>` addresses one instance deliberately.
- **`nats service serve` is a demo, not a host.** It builds a service named after its argument with a
  group of the same name and a single `echo` endpoint, so the subject is always `<name>.echo`
  (`cli/service_command.go:147–182`). It cannot host an endpoint on a subject you choose.
- **Ctrl-C on `nats service serve` does not drain.** `serveAction` blocks on `<-ctx.Done()` and returns
  without calling `Stop()`, so the connection closes abruptly and in-flight work is lost. The
  graceful stop is a client-library call; there is no CLI equivalent.

`nats service list --json` prints the raw `info_response` bodies, and `stats --json` the raw
`stats_response` bodies, which is the scrape surface for a service's counters — there is no Prometheus
bridge for them. For the bodies without the wrapper: `nats request '$SRV.INFO' '' --replies=0`
(`--replies=0` waits out `--timeout` and prints every reply).


## What bites you — `nats pub -J` is a diagnostic, not a resilient publisher

Read at **natscli v0.4.0**, `cli/pub_command.go:253–290` (`raw/nats-go-src/jetstream-publish-v1.53.1.md`):

- **It does not use `js.Publish`.** The whole JetStream path is `resp, err := nc.RequestMsg(msg,
  opts().Timeout)` followed by `jsm.ParsePubAck(resp)` — a plain core request. So none of
  [[s-adr-22-publish-retries]]'s back-off runs: no `RetryWait`, no `RetryAttempts`, no retry at all.
  On a stream-leader step-down the CLI failed in 14 ms including process start, where a retrying
  client would have taken at least 500 ms (source: [[s-nats-server-core-or-jetstream-observed]]).
- **The error string tells you which you have.** `nats: no responders available for request` is a
  client that did not retry; `nats: no response from stream` is nats.go after exhausting its retries.
- **It prints `Published N bytes …` before it sends the request** (`:277` precedes `:279`), so a
  failing JetStream publish prints a success line and then an error:
...
- Consequence for benchmarking and for runbooks: `nats pub -J` measures one round trip with no
  resilience. It is the right tool for "is this subject captured by a stream" and the wrong one for
  "would my application have survived this election".

`nats bench pub` versus `nats bench js pub sync|async` is the honest comparison, and it is on
[[core-or-jetstream]].
...
- [[s-docs-core-nats-request-reply]] · [[s-docs-core-nats-queue-groups]] · [[s-adr-47-request-many]] — the
  gather flags as the docs and ADR-47 describe them, for the `nats request` section. · [[s-nats-cli-reconnect]] · [[s-nats-server-client-lifecycle-observed]] · [[s-docs-resilient-clients-drain-and-shutdown]] · [[s-docs-resilient-clients-connecting]] · [[s-docs-resilient-clients-reconnection-and-events]] · [[s-docs-resilient-clients-tls-and-auth]] · [[s-nats-server-client-faults-observed]] · [[s-adr-32-service-api]] · [[s-docs-services-discovery-and-stats]] · [[s-docs-services-framework]] · [[s-nats-server-services-observed]] · [[s-adr-22-publish-retries]] · [[s-nats-server-core-or-jetstream-observed]]
