...
| 2.11.15 | 2026-03-24 | 1.25.8 | ★ | **ten CVEs** (MQTT ×3, leafnodes ×2, WebSockets, JetStream ×2, mutual TLS, command-line credentials, publish permissions); the line's one `### Changed`: **a 1 MB size limit on JWTs** (#7960); `peer-remove` accepts a peer ID; a path-traversal bug on account purge; `Nats-Trace-Dest` now needs publish permission The CVE for "leafnodes and service imports" is the `Nats-Request-Info` spoofing (CVE-2026-33246): the body's fix line is "Messages from leafnodes to non-shared service imports now correctly rebuild the request info header" — see [[service-import-request-info]]. |
