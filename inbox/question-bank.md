...
| 108 | One stream per tenant or service, or one stream with a tenant prefix in the subject — which, and at what count does "many streams" start to cost you (meta layer, account limits, consumers)? | jetstream security | [gh#6100](https://github.com/nats-io/nats-server/discussions/6100) | design pattern sizing | [[stream-topology-design]] — *How many streams* and *One stream per tenant*, with the `R1`/`R3` costs measured; the two public answers point opposite ways and neither was measured |
| 109 | How should I design a subject hierarchy for JetStream — token order, where the wildcards go, and what subject cardinality costs the filestore and filtered consumers? | jetstream core | [gh#4170](https://github.com/nats-io/nats-server/discussions/4170) | design pattern | [[subject-design]] |
| 110 | Limits, Interest or WorkQueue — how do I choose retention for a task queue, an event log and a cache, and what breaks when I choose wrong? | jetstream | [gh#4499](https://github.com/nats-io/nats-server/discussions/4499) | design concept | [[retention-policies]] — *Choosing retention: a task queue, an event log, a cache*, the two questions in order and *What breaks on each wrong choice* |
| 111 | Filtered consumers on one large stream, or many small streams — which scales better for fan-out to N services, and where do `max_consumers` and per-consumer state bite? | jetstream | [gh#3405](https://github.com/nats-io/nats-server/discussions/3405) | design pattern sizing | [[stream-topology-design]] — *How many consumers, and on what*: the 300-way fan-out built both ways (2.75× the RSS, 1.5× less ingest for many streams), and *Tenancy* for `max_consumers` |
| 112 | `Nats-Msg-Id` deduplication or `Nats-Expected-Last-Subject-Sequence` — which gives me exactly-once-ish publishing for my case, and what does each cost? | jetstream | own | design pattern | [[publishing]] — *The five expectation headers, and the one counter behind them*: dedup drops a repeat, an expectation refuses a wrong one; five headers, one stream counter filtered three ways, and the six rejection codes on [[error-codes]] · [[stream]] |
...
| 114 | Mirror, source, or a filtered consumer on the original stream — which replication shape for read replicas, fan-in and cross-region copies, and what does each cost? | jetstream topology | [gh#6571](https://github.com/nats-io/nats-server/discussions/6571) | design pattern | [[stream-topology-design]] — *How a second copy is made*: mirror against source at 1.38–1.57×, and **the third shape does not exist server-side** (SI-9) |
...
| 144 | Is JetStream suitable as an event store — millions of events, one subject per aggregate, optimistic concurrency on publish, and tiering to cold storage? | jetstream | [gh#3772](https://github.com/nats-io/nats-server/discussions/3772) | design pattern | [[event-sourcing-on-jetstream]] — yes for the first three, with the 2.11+ header a multi-subject aggregate needs and what a million aggregates cost; **no** for the fourth, and *The dead end* says so with dates |
...
| 198 | Does JetStream have tiered storage — can sealed blocks be offloaded to S3 or a slower tier the way Kafka and Pulsar do it? | jetstream deploy | [gh#3871](https://github.com/nats-io/nats-server/discussions/3871) | design interop | **No** — [[event-sourcing-on-jetstream]], *The dead end: there is no tiered storage*: "planned but no schedule" (2023-02-16) and never answered again, the 42-upvote idea with no maintainer in it, the rclone mount that worked and then corrupted, and the two shapes that do exist (an archiving consumer, sharding by time). Also [[stream]] — *And there is no tier below them either* and [[kubernetes-storage]] — *Not an object-store mount either* |
