...
## Step 5 — the wire protocol: `reference/wire-protocol` · status: done 2026-09-04 — s-docs-protocol-client, s-docs-protocols-internal, s-nats-server-wire-protocol (three, as planned); page `reference/wire-protocol`; **16 ripples** (`gateway`, `leafnode`, `client-connection-lifecycle`, `client-defaults`, `core-nats-delivery`, `defaults-and-limits`, `error-codes`, `subject-permissions`, `tls-in-nats`, `monitoring-endpoints`, `how-clients-reach-a-cluster`, `build-a-3-node-cluster`, `system-subjects`, `duplicate-messages-across-a-leafnode`, `supercluster-slows-when-a-remote-subscriber-joins`, `nats-server`, `nats-server-2.10`, plus the two step-4 gotchas — unlanded 21 → 0); docs issues **#102–#107**, one per protocol page plus two for `client.md`, no server issue; rows **183–187** added and answered (155 / 187, `own` 32); extract `wire-protocol-v2.14.6.md` (30 ranges) and runs **A–G in seven passes** (`wire-protocol-observed-v2.14.6.md`, `wire-protocol-raw.py` + nine scripts). **What the runs settled**: the standalone client `INFO` carries `api_lvl` and `xkey`, which no doc row has (A1); an `INFO` line ends with a **space before its CRLF** except on a gateway, because `generateInfoJSON` joins three pieces with `" "` (`util.go:360–364`, A1–A5); the leafnode and route listeners advertise `proto: 3`, not 1 (A3/A4); **`CONNECT {}` gets a `+OK`** — `defaultOpts` seeds `Verbose`/`Pedantic`/`Echo` to true (B2/B13); a CONNECT with broken JSON gets **no `-ERR` at all** (B9); `Authentication Timeout` (not `Authorization Timeout`) at 1003 ms with `timeout: 1` (C15); a TLS-required server sends **nothing** and drops at 2002.8 ms, because `TLSHandshakeError` is in `markConnAsClosed`'s skip-flush set — so `Secure Connection - TLS Required` is unreachable (C16); `Stale Connection` at **6139.9 ms** with `2s`/`2`, i.e. `(ping_max + 1) × ping_interval` (C17); four `-ERR`s leave the connection **usable** (C2/C3/C11/C19); the `-ERR` inventory is **60** sites, not 58 — `Stale Connection` bypasses `sendErr` (`client.go:5867`, `:5908`); a client on a leafnode port gets `Authorization Violation` when the port needs auth and `attempted to connect to leaf node port` when it does not (D2/E1); `$LDS.` arrives as an ordinary `LS+` — there is no `LDS` verb (E1b); the origin cluster is the **first** token of a route's `LS+`/`LS-` (`LS+ LEAF1 $G edge.ping`, E/F); a leaf's header messages are **`HMSG`**, and `LMSG` has a three-token reply form with no `+` (F); the gateway mapped reply is `_GR_.<cluster hash>.<server hash>.<reply>` (G); and **`gateway_iom: true`** — interest-only by default since 2.9.0, so the optimistic path the docs describe is never entered (A5, `gateway.go:552–558`). Lint 397 pages, 0 · 0, wanted **1** (`services-on-core-nats`, registered for step 6), unverified 12, staleness 0 behind. **Carried to step 6**: `services-on-core-nats` is a registered wanted page and `wire-protocol`'s `$SRV.` row links to it; `A+` / `A-` and the `gateway_cmd` 1/2/3 sequence were **not** observed (both sides are interest-only from the first frame) and are listed under *What was not tested*, along with WebSocket, compression actually engaged, MQTT and pinned-account routes
...
## Step 6 — the services framework · status: done 2026-09-04 — s-adr-32-service-api, s-docs-services-framework, s-docs-services-discovery-and-stats, s-docs-services-scaling, s-nats-server-services-observed, s-gh-4984-micro-with-jetstream (**six, not five**: the (b) scout of the comment cache found only two `micro` hits in 484 threads, and gh#4984 is the public statement of where the pattern stops, so it was fetched whole rather than merely cited); pages `services-framework` and `services-on-core-nats` (**row 134**, the last of the phase's three); **12 ripples**, unlanded 33 → 0 (`queue-groups`, `request-reply`, `system-subjects` — whose `$SRV.` row said "(step 6)" and now points at the page — `nats-timeout`, `worker-pool`, `leafnode`, `subject-permissions`, `nats-cli`, `nats-go`, `advisories`, `metrics`, `core-nats-delivery`, `monitoring-endpoints`, `client-connection-lifecycle`, `slow-consumer-in-the-client`); docs issues **#108–#117**, ten from one chapter with **#113** and **#114** ★, plus a harder run added to **#86**; no server issue; rows **188–193** added and answered, **162 / 193**, `own` 37; the micro schemas promoted into `raw/jsm-go/`; adr-toc row 32 linked and row **3** given a *skip* decision. Runs **A–E in six passes** (`services-observed-v2.14.6.md`, eight scripts, `services-svc.go` — a six-endpoint nats.go v1.53.1 service — `services-raw.py`, `services-subsz.py`). **What the runs settled**: `$SRV` is nowhere in the server, so the framework is entirely a client convention; **ten subscriptions per instance** (three verbs × three levels, all plain, plus one queue subscription per endpoint) — which is *why* discovery is a broadcast; a `ping_response` is five fields and nothing else, and `stats_response` carries UTC `started`, integer-nanosecond times, `last_error` as `""`, and `"<code>:<description>"` when set; a **service error and a 503 side by side on the wire** (`HMSG … 92 109` with two headers and a body against `HMSG … 55 55` with `NATS/1.0 503` and none); **a blocked endpoint does not block its siblings** (327 µs and 508 µs while `slow` was 3 s into a block) because nats.go gives each endpoint its own `QueueSubscribe`; **a blocked instance keeps its share** — two instances, 3 s handlers, eight simultaneous requests, all delivered at once, split 3 / 5 at random, **four of eight callers timed out** while replies landed at 20.09 / 23.09 / 26.09 s in dead inboxes; **`Stop()` removes all fifteen subscriptions at once and returns in 1.3 ms**, the in-flight handler still replying at 5.001 s because the process stayed alive, while `kill -9` loses it with no no-responders answer at all; **the server does not reserve `$SRV`** (`Published 5 bytes to "$SRV.PING"` from an ordinary client, and `$SRV.>` eavesdropping works), so permissions are the only isolation and a refusal is invisible to the client; and **`$SRV` and endpoint subjects cross a leafnode with no configuration while the queue group stays local** — 8 of 8 each way. Lint 405 pages, 0 · 0, wanted **1** (`core-or-jetstream`, registered for step 7), unverified 12, staleness 0 behind. **Carried to step 7**: `raw/gh-discussions/gh-4984.md` is already in the tree and is the thread `inbox/scout-backlog.md` names as row 133's nearest, so step 7 starts from it rather than re-fetching; `core-or-jetstream` is linked from `core-nats-delivery` (*The at-most-once boundary, asked in public*) and from `services-on-core-nats` (*When not to use it*), both of which stop at the durability boundary and hand the decision on. **Not tested** and listed in the observed file: any client but nats.go and natscli, `$SRV` across a gateway or an account import, `Reset()`, the `DoneHandler`, endpoints added after start, and whether an endpoint subscription can reach a slow-consumer condition
...
## Step 7 — `core-or-jetstream` · status: done 2026-09-04 — s-docs-concepts-jetstream, s-docs-core-nats-chapter, s-docs-jetstream-where-next, s-gh-2961-js-and-core-one-cluster, s-adr-22-publish-retries, s-nats-server-core-or-jetstream-observed, s-gh-3507-no-external-store (**seven, not four**: the scout found no thread that asks row 133, so the page rests on the docs' own two statements of the rule, the boundary ADR, the one public maintainer line on the mixed deployment and the runs — and the seventh was a **repair**, gh#3507 being cited by bank row 143 on two pages that never stated its fact); page `operations/core-or-jetstream` (**row 133**, closes megaplan **G7**, and the wiki's last registered wanted page); **15 ripples**, unlanded 25 → 0 (`core-nats-delivery`, `publishing`, `stream` ×2, `consumer`, `ack-and-redelivery`, `request-reply`, `nats-timeout`, `nats-cli`, `nats-go`, `jetstream-sizing`, `stream-and-consumer-config`, `services-on-core-nats`, `worker-pool`, `subjects-and-wildcards`, `choosing-a-topology`, `nats-streaming`); docs issues **#118–#120** with **#119 ★**; no server issue; rows **194–197** added and answered, 133 filled, 143 re-pointed — **167 / 197**, `own` 39; adr-toc row 22 linked. Scout `inbox/scout-core-or-jetstream-2026-09-04.md` (ten candidates; the comment cache re-swept with different terms, the SE API, natsbyexample's index — row 133 keeps `own`, searched twice). Runs **A–H in eight passes** (`core-or-jetstream-observed-v2.14.6.md`, nine scripts, the step-3 raw client unchanged, run G on the lab). **What the runs settled**: a JetStream publish is a core publish with a reply subject, and once stored the two messages are indistinguishable; the cost as ratios on one laptop — core 2,882,347 msgs/s, JetStream sync **30,876** (~93×), async 366,110 (~7.9×), async into memory 591,419, and a core publish into a captured subject **2,885,763, unslowed**; 100,000 core publishes into a file stream stored 100,000 with no drop and no log line; **a stream laid over a request/reply subject answers the requests itself** with its `PubAck`, first (277 µs against 451 µs), as two indistinguishable `MSG` frames on one inbox, while storing every request — and the server refuses only `>`, `$JS.>`/`$JSC.>`/`$NRG.>` and `$SYS.>`, each only by demanding `no_ack` (`server/stream.go:2170–2196`); **the docs' own Acme example does exactly this to itself** (run H: after `nats stream add ORDERS --subjects "orders.>" --defaults`, `orders.inventory.check` answers `{"stream":"ORDERS","seq":1}` instead of `in stock: 42`) — docs issue #119 ★; a `>` stream needs `no_ack` **and** R1, swallowed twelve subjects and 41 messages from six commands including `$JS.API.STREAM.INFO.<itself>` (3 → 6 between two `stream subjects` calls — reading it writes to it), and can never accept a JetStream publish; a stream-leader step-down cost an R3 JetStream publisher **exactly one publish of 312** while the core publisher saw nothing and a **meta**-leader step-down cost nothing; and **`nats pub -J` never retries** (`cli/pub_command.go:279` is a plain `nc.RequestMsg`, and `:277` prints `Published …` before sending), though ADR-22's own `250ms` × 2 defaults do hold in nats.go v1.53.1 in both APIs. Lint 413 pages, 0 · 0, **wanted 0**, unverified 12, staleness 0 behind. **Carried to step 8**: `raw/nats-go-src/jetstream-publish-v1.53.1.md` now holds a natscli range too, so the `nats-cli` entity has one *what bites you* section already written; `local/scratch/src/nats.go-v1.53.1/` gained `js.go`, `jetstream-publish.go` and `jetstream-jetstream.go` at the pin. **Not tested** and listed in the observed file: any client but nats.go and natscli, whether every client library races the same way in run H, the hijack across an account import or a leafnode, whether a mirror or source behaves the same, the async publisher's in-flight window, and whether run G's 622-against-623 message count is lag or loss
