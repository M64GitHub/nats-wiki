...
**Ingested 2026-09-03** (step 4): candidates 1 → `s-gh-8001-jetstream-startup-slow-50m`; 2 + 4 (+ 3 quoted) → `s-nats-server-filestore-recovery` (`raw/nats-server-src/filestore-recovery-v2.14.6.md`); 6 → `s-gh-8333-high-cardinality-subjects`; 7 → `s-gh-5202-max-unique-subjects`; 8 → `s-synadia-how-many-subjects` (the blog into `raw/synadia-blog/`, the two check pages into `raw/synadia-insights/`); 9 → cited from the source summary on `nats-server-2.14`, `defaults-and-limits`, `consumer-slow-on-a-sparse-stream`; 10 → docs issues **#52** and **#53**; 11 → `s-gh-7147-one-billion-cap`; 12 → `s-gh-7032-max-msgs-known-good`; 13 → pointer for phase G (still in `local/scratch/gh/`); 14 skipped; 15 → the baseline sentence kept on `filestore-layout`; runs D, E, F → `s-nats-server-stream-scale-observed` (`raw/nats-server-src/stream-scale-observed-v2.14.6.md`). The gotcha is `jetstream-recovery-is-slow`. Rows 4, 5, 9, 13 answered. The expected `SI-3` (the `Restored … in` attribution) was recorded as fact on the pages instead; the `SI-3` that was filed is a `*` inside a token (`inbox/server-issues.md`), found by run E.
