...
## Step 2 — the runs, and the ingest for scout 1 · status: open — the pick is recorded in the scout's *Status* line (candidates 1, 2, 4, 5, 6, 7, 8 to ingest; 3, 9, 10 local checks; 11, 12 pointers). Nothing from the scout session survives in `raw/` yet: re-fetch gh#8417, gh#8444 and issue #5106 through the GraphQL API into `raw/gh-discussions/` and `raw/gh-issues/` (the header form of `raw/gh-discussions/gh-7533.md`), and quote the `stream.go` / `filestore.go` ranges at v2.14.6 into `raw/nats-server-src/mirror-v2.14.6.md` — the line numbers are in the scout. Save the v2.14.1–v2.14.6 release bodies into `raw/release-notes/` while at it (phase D needs them anyway; one manifest note).
