...
**Status:** **picked 2026-09-02 by the user — the recommendation as written:** ingest candidates
**1, 2, 4, 5, 6, 7, 8**; run **3, 9, 10** as local checks (10 becomes a docs issue only if it verifies);
**11, 12** as dated pointer sentences, not summaries. Runs A, B, C as named above. Not yet ingested —
step 2 of `inbox/plan-the-runnable-scouts-2026-09-02.md` does it (`start the megaplan` in a fresh
session lands there). When the summaries exist, replace this line with the candidate → summary map.
