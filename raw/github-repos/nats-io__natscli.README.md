<!-- source: https://github.com/nats-io/natscli/blob/main/README.md · fetched 2026-08-31 · README.md of nats-io/natscli -->
...
To produce load at a fixed rate (useful for head-to-head comparisons against tools like Kafka's `kafka-producer-perf-test.sh --throughput`), use `--throughput` on any generator-side bench command (`bench pub`, `bench service request`, `bench js pub sync|async|atomic|fast`, `bench kv put`). The rate is the aggregate target across all clients; each client targets `throughput / clients` msgs/sec. A value of `0` (the default) disables throttling. If `--sleep` is also set, both limits apply and the actual send rate will be the slower of the two, so `--sleep` can prevent reaching the configured `--throughput`.
