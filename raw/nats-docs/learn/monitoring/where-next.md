<!-- source: https://docs.nats.io/learn/monitoring/where-next.md · fetched 2026-08-31 · section: where-next -->
# Where to go next
...
The [Reference root](/reference/.md) is the entry point. The handoff phrases throughout this chapter ("the full set of fields is documented in Reference") all point into it. The [monitoring endpoints reference](/reference/system/monitor/.md) and the [advisory reference](/reference/system/advisory/.md) are the two you'll reach for most.
...
The [Backup & Recovery deep dive](/learn/backup-recovery/.md) is what you reach for when the lag you measured here won't drain on its own. It covers snapshotting a stream and restoring it, the repair this chapter deliberately never prescribes.

The [Deployment deep dive](/learn/deployment/.md) covers running the things you watched: sizing and running the exporter and Prometheus, and scaling the `shipping` consumer pool when its lag climbs for real.

The [Clustering & Replication deep dive](/learn/clustering/.md) explains the mechanics behind the events you only *observed* here. When a leader-elected advisory reports a flap, the *why* lives in that chapter's [raft-and-leaders](/learn/clustering/raft-and-leaders.md) page.

The [Services deep dive](/learn/services/.md) and its [observability](/learn/services/observability.md) page extend the metric picture to request/reply: service latency and error counts, the one metric family this chapter pointed at but didn't cover.
...
You hold the core model: the live numbers come from the monitoring endpoints, lag comes from consumer state, the events you didn't poll for come from advisories, and the history that turns a number into an alert comes from the exporter. Those four sources together are how you keep a NATS deployment observable in production. When all four say a node is unhealthy and none says why, a [profile](/learn/monitoring/profiling.md) is what you take next.
...
### Monitoring endpoints — see [Pitfalls](/learn/monitoring/monitoring-endpoints.md#pitfalls)
...
* <!-- -->
  Restrict the monitoring port; it's unauthenticated by default (see [Security](/learn/security/.md)).

### JetStream health — see [Pitfalls](/learn/monitoring/jetstream-health.md#pitfalls)
...
### Advisories and events — see [Pitfalls](/learn/monitoring/advisories-and-events.md#pitfalls)
...
* <!-- -->
  Read a leader-elected advisory as a flap report only; the *why* behind the election is [Clustering](/learn/clustering/.md).

### Prometheus and dashboards — see [Pitfalls](/learn/monitoring/prometheus-and-dashboards.md#pitfalls)
...
### Profiling — see [Pitfalls](/learn/monitoring/profiling.md#pitfalls)
...
* [Reference → monitoring endpoints](/reference/system/monitor/.md) — every endpoint field, type, and query param, versioned and exhaustive.
* [Backup & Recovery deep dive](/learn/backup-recovery/.md) — the fix for a lag this chapter only measured.
* [Clustering & Replication deep dive](/learn/clustering/.md) — the mechanics behind the leader-elected advisories you observed.
