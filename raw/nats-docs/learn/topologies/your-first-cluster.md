<!-- source: https://docs.nats.io/learn/topologies/your-first-cluster.md · fetched 2026-08-31 · section: your-first-cluster -->
# Your first cluster

The [previous page](/learn/topologies/single-server.md) left Acme running on one server, `n1`. That server publishes `orders.*` and holds the `ORDERS` stream. It also has one critical limitation: if it stops, the whole ORDERS system stops with it.
...
**Message flow — One seed grows into a full mesh (animated):** The east cluster: three servers meshed by routes, one order hopping from n1-east down to n2-east

* n1-east → n3-east
* n2-east → n3-east
* order-svc → n1-east (subject: orders.created)
* n2-east → warehouse

The wire-level detail of how two servers open a route, exchange subscriptions, and forward messages is documented in [Reference → Route protocol](/reference/protocols/route.md). We only need the config and the shape here.

## Configure three servers

Each server in `east` needs the same cluster name and its own pair of ports. Routes connect the servers, but the cluster name has to match for them to join: a mismatched name (a typo like `eats`) gets that server's route refused — the log contains `cluster name "eats" does not match "east"` — and it ends up a cluster of its own.
...
Each config also carries a `jetstream` block with its own `store_dir`, so the cluster can hold the `ORDERS` stream the workload has been using. It's only switched on here — what JetStream actually does once it runs across a cluster is the [next page](/learn/topologies/jetstream-in-a-cluster.md). This page is about the routes.
...
Confirm a message crosses the cluster: subscribe on one server, publish on another, and watch it arrive.
...
Next, test failover. Point a publisher at one specific server — `n1-east` — send a steady stream of orders, then kill `n1-east` out from under it.
...
Let a few go by, then kill `n1-east`: `kill %1` in the terminal where you started the servers, or `Ctrl+C` its window if you gave it one. The publisher doesn't stop. With `--trace` it logs each connection event, so you can watch it drop `n1-east` and reconnect (a couple of discovery lines are trimmed below):
...
**Plan for an even server count.** A cluster of two or four servers works fine for plain `orders.*` traffic, but the moment you replicate the `ORDERS` stream you want an *odd* count. A two-server set loses its majority as soon as one server is down, and a four-server set tolerates only one loss — the same as three — so the extra server adds cost without buying more failure tolerance. That's a JetStream concern, covered on the [next page](/learn/topologies/jetstream-in-a-cluster.md); the consensus math behind it lives in [Clustering & Replication](/learn/clustering/.md). For a pure routing cluster, any count is fine.
...
What you watched cross the cluster was plain pub/sub — the durable `ORDERS` stream isn't on `east` yet. The next page creates it on the cluster, replicated across the three servers so it survives a server loss the way the cluster already does: [JetStream in a cluster](/learn/topologies/jetstream-in-a-cluster.md).

## See also

* [Reference → Route protocol](/reference/protocols/route.md) — the wire-level detail of how servers form routes and forward messages.
* [Operate → Clustering & Replication](/learn/clustering/.md) — Raft, leader election, and replica placement inside a cluster.
* [Core Concepts → Topologies](/concepts/topologies.md) — the five-minute overview of every shape.
