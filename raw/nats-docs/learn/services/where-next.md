<!-- source: https://docs.nats.io/learn/services/where-next.md · fetched 2026-08-31 · section: where-next -->
# Where to go next
...
The [Reference root](/reference/.md) is the entry point. The handoff phrases throughout this chapter ("the full set of service configuration fields is documented in Reference") all point into it. The `$SRV` wire format and its PING, INFO, and STATS response schemas live under [the services reference](/reference/services/.md).
...
The [Core NATS deep dive](/learn/core-nats/.md) covers the lower layer this one builds on. [Request-reply](/learn/core-nats/request-reply.md) is the mechanism every endpoint uses, and [queue groups](/learn/core-nats/queue-groups.md) are exactly the load balancing that the default group `"q"` gives you. If anything in this chapter seemed to skip a step, that step is covered there.
...
The [Resilient Clients deep dive](/learn/resilient-clients/.md) covers the connection underneath every instance: reconnect, drain beyond the framework's `Stop()`, slow-consumer handling, and error callbacks. This chapter assumed a healthy local connection; production connections aren't always healthy.

The [Security deep dive](/learn/security/.md) covers subject isolation for `$SRV` subjects, account access control, and cross-account service placement.

The [Monitoring deep dive](/learn/monitoring/.md) goes past per-endpoint stats to the server-side view: exporting metrics to Prometheus and building the dashboards that tell you a service is slow before a caller complains. The service-latency advisory schema itself lives in [Reference](/reference/services/.md).

The [Topologies deep dive](/learn/topologies/.md) covers running services across more than one server: how `$SRV` and service subjects propagate across leaf nodes and gateways, and how to place instances in more than one region.
...
### Your first service — see [Pitfalls](/learn/services/your-first-service.md#pitfalls)
...
### Endpoints and groups — see [Pitfalls](/learn/services/endpoints-and-groups.md#pitfalls)
...
### Discovery — see [Pitfalls](/learn/services/discovery.md#pitfalls)
...
### Observability — see [Pitfalls](/learn/services/observability.md#pitfalls)
...
### Scaling — see [Pitfalls](/learn/services/scaling.md#pitfalls)
...
* [Reference](/reference/.md) — every service config field, endpoint option, and `$SRV` wire schema, versioned and exhaustive.
* [Core NATS deep dive](/learn/core-nats/.md) — the request-reply and queue-group foundation this chapter is built on.
* [JetStream deep dive](/learn/jetstream/.md) — where service state goes when it has to survive a restart.
