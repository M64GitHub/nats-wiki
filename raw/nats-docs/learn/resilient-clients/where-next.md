<!-- source: https://docs.nats.io/learn/resilient-clients/where-next.md · fetched 2026-08-31 · section: where-next -->
# Where Next
...
Most pages added exactly one transition the Acme clients couldn't handle before. Connecting taught the CONNECTING → CONNECTED edge and the handshake that walks it. Reconnection taught the CONNECTED → RECONNECTING → CONNECTED loop with backoff and jitter. Connection Events added no new edge; it made every transition observable, wiring each one to a callback and the live state to a health check. Drain & Shutdown taught the CONNECTED → DRAINING → CLOSED edge that loses no work. Slow Consumers kept a CONNECTED client healthy under load instead of letting a subscriber's buffer grow without bound. Request-Reply Resilience made a single `request()` call survive a slow or absent responder. TLS & Auth secured the edge into CONNECTED so the link is encrypted and the client is who it claims to be.

Those six mechanisms all act on one machine, and Connection Events makes that machine observable. Everything else is a refinement of those edges: the exact flags, the defaults, the per-language spelling.

## Where the details live

The chapter is unversioned and concept-first. The wire protocol, the server's `-ERR` strings, and the server configuration live in [Reference](/reference/.md), which is versioned. The exact client option names, types, and defaults live in each client library's API reference.

Here we covered only the options that change how a connection behaves under fault. For the full list of error codes a `-ERR` can carry, look in Reference; for a connection option's precise type or default, look in your client library's API docs.
...
The [Topologies deep dive](/learn/topologies/.md) explains the server pool this chapter only connects to: why a server goes away, how the `n1`/`n2`/`n3` cluster forms (built there as `n1-east`/`n2-east`/`n3-east`, shortened here), and what a client's disconnect looks like from the server side. Resilient Clients treats "the server is gone" as a given fact, while Topologies explains why it happens.

The [JetStream deep dive](/learn/jetstream/.md) owns what happens to a consumer's *position* across a reconnect. This chapter re-subscribes the connection; [JetStream → Acknowledgment](/learn/jetstream/acknowledgment.md) covers what a consumer's position does and whether in-flight work is repeated or resumed.

The [Security deep dive](/learn/security/.md) issues the credentials and the CA this chapter loads. TLS & Auth *consumes* the `order-svc` `.creds` and the cluster CA; Security shows how both are created.

The [Services deep dive](/learn/services/.md) formalizes the request-reply pattern into a named, discoverable service with per-endpoint stats and queue-group scaling. If your responder side has outgrown a single hand-rolled handler, that's the next step.

The [Monitoring deep dive](/learn/monitoring/.md) watches the same connections from the server side: the `slow_consumers` metric and the advisories that show a client falling behind, and the health endpoints that watch the servers themselves.
...
### Connecting — see [Pitfalls](/learn/resilient-clients/connecting.md#pitfalls)
...
### Reconnection — see [Pitfalls](/learn/resilient-clients/reconnection.md#pitfalls)
...
### Connection Events — see [Pitfalls](/learn/resilient-clients/connection-events.md#pitfalls)
...
### Drain & Shutdown — see [Pitfalls](/learn/resilient-clients/drain-and-shutdown.md#pitfalls)
...
### Slow Consumers — see [Pitfalls](/learn/resilient-clients/slow-consumers.md#pitfalls)
...
### Request-Reply Resilience — see [Pitfalls](/learn/resilient-clients/request-reply-resilience.md#pitfalls)
...
### TLS & Auth — see [Pitfalls](/learn/resilient-clients/tls-and-auth.md#pitfalls)
...
* [Reference](/reference/.md) — the wire protocol, `-ERR` strings, and server configuration, versioned.
* [Topologies deep dive](/learn/topologies/.md) — the server pool this chapter only connects to, explained from the server side.
* [Security deep dive](/learn/security/.md) — issuing the credentials and CA this chapter consumes.
